...
ConfigBuilder::ConfigBuilder(toml::v3::table config) {
    ...
    if (type == Config::Strategy::KeyValue) {
        ConfigBuilder::KeyValueBuilder key_value_builder = ConfigBuilder::KeyValueBuilder().strict(config["strict"].value_or(false));
...
ConfigBuilder::KeyValueBuilder& ConfigBuilder::KeyValueBuilder::with(std::string key, std::filesystem::path value) {
    key_value._mapping.insert({ key, value });
    return *this;
}

ConfigBuilder::KeyValueBuilder& ConfigBuilder::KeyValueBuilder::strict(bool value) {
